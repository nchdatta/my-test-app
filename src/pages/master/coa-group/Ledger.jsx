...
import COALedgerDialog from '../../../components/Dialogs/COALedgerDialog';
import ViewCOALedgerModal from '../../../components/Modals/ViewCOALedgerModal';
...
const Ledger = () => {
    ...
    return (
        <Page title='COA Ledger Management'>
            <PageHeader
                title="COA Ledger Management"
                btnText="Add New"
                handleBtn={() => {
                    setRowData(null);
                    setAddNewDialog(true);
                }}
                currentPage="COA Ledger Management" />


            <Filter placeholder="Search Ledger..." enableExport />
                ...
                <Table size="medium" sx={{ minWidth: 650, minHeight: '70vh' }} aria-label="simple table">
                    <TableHead>
                        <TableRow sx={{ bgcolor: grey[100] }}>
                            ...
                            <TableCell align="left" sx={{ fontWeight: "bold" }}>Sub-Group</TableCell>
                            <TableCell align="left" sx={{ fontWeight: "bold" }}>Group</TableCell>
                    ...
                    <TableBody>
                        {financialYearsData.map((row, index) => (
                            <TableRow key={index}>
                                ...
                                <TableCell align="left">{101010001}</TableCell>
                                <TableCell align="left">{"APM Global Logistics Bangladesh Ltd"}</TableCell>
            ...
            {addNewDialog && <COALedgerDialog setOpenDialog={setAddNewDialog} openDialog={addNewDialog} metadata={{ id: rowData?.id }} />}
            {viewModal && <ViewCOALedgerModal
                setOpenModal={setViewModal}
                openModal={viewModal}
                metadata={{ title: 'COA Ledger Information', data: rowData }} />}
